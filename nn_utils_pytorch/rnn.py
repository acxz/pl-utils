...
# FIXME maybe easy way to reduce duplicate code in trainRNN and testRNN
def trainRNN(model, device, train_dataloader, optimizer, loss_func,
    ...
    for batch_num, (input_, target) in enumerate(train_dataloader):
        ...
        # Predict
        output, final_hidden_value = model(model_input, initial_hidden_value)
...
def testRNN(model, device, test_dataloader, loss_func, accuracy_func,
...
# Define a custom Dataset class to facilitate training for neural net models
...
# FIXME
def check_accuracy(value_1, value_2):
    is_accurate = False
    if (torch.allclose(value_1, value_2, rtol=1e-05, atol=1e-08)):
        is_accurate = True
    return is_accurate
...
def main():
    ...
    for epoch in range(0, epochs):
        ...
        #trainRNN(model, device, train_dataloader, optimizer, loss_func,
        #        accuracy_func, epoch_display_status, epoch, epochs,
        #        batch_display_interval)
        testRNN(model, device, test_dataloader, loss_func, accuracy_func,
